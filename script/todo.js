        ...
        $.get("system/getTodo.php", function (data, success) {
            ...
            for (var todo_item = 0; todo_item < todoLists.lists.length; todo_item++) {
                ...
                if(todoLists.lists[todo_item].STATUS == 0)
                    $("#list_undone").append("<li id=\"todo_" + todoLists.lists[todo_item].ID + "\" class=\"list-group-item\"><div class='checkbox'><label><input onclick=\"eventHan(this);\" class=\"check_todo_lists\" type='checkbox' name=\"check_todo_lists\" value='" + todoLists.lists[todo_item].ID + "'>" + todoLists.lists[todo_item].TOPIC + "<\/label><\/div><\/li>");
            }
            if(call_undone){
